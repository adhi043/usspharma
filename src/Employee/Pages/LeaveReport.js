    ...
    useCallback(async () => {
      ...
      axios.get(`${baseUrl}/employe/get/${mydata?.id}`).then(res => {
        if (res.data.status === 'ok') {
          ...
          axios.get(`${baseUrl}/employeLeave/byemploye/${res.data.data.id}`).then(resp => {
      ...
      <View style={{ padding: 20, paddingTop: 40, paddingHorizontal: 10 }}>
        ...
        <View style={[tw`flex-row justify-between items-center mb-1`, { paddingBottom: 10 }]}>
          <View style={tw`flex-row justify-between items-center gap-x-3`}>
            ...
            <View>
              <Text style={{ color: Color.white, fontSize: FontSize.headline3_size, fontWeight: '800' }}>Leave Report</Text>
            </View>
        ...
        <View style={{ marginVertical: 10, flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 30 }}>
          ...
          <TouchableOpacity style={[tw`p-2`, { backgroundColor: sel === 'Pending' ? Color.gray2 : Color.background2, borderRadius: 20, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', minWidth: 70, gap: 5, elevation: 5, borderColor: Color.white, borderWidth: 1 }]} onPress={() => {
            setSel('Pending')
          }}>
            <Text style={{ color: sel === 'Pending' ? Color.white : Color.white, fontSize: FontSize.font1_size }}>Pending</Text>
          </TouchableOpacity>


          <TouchableOpacity style={[tw`p-2`, { backgroundColor: sel === 'Rejected' ? Color.error : Color.background2, borderRadius: 20, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', minWidth: 70, gap: 5, elevation: 5, borderColor: Color.white, borderWidth: 1 }]} onPress={() => {
            setSel('Rejected')
          }}>
            <Text style={{ color: sel === 'Rejected' ? Color.white : Color.white, fontSize: FontSize.font1_size }}>Rejected</Text>
          </TouchableOpacity>




          <TouchableOpacity style={[tw`p-2`, { backgroundColor: sel === 'Approved' ? Color.colorMediumseagreen : Color.background2, borderRadius: 20, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', minWidth: 70, gap: 5, elevation: 5, borderColor: Color.white, borderWidth: 1 }]} onPress={() => {
            setSel('Approved')
          }}>
            <Text style={{ color: sel === 'Approved' ? Color.white : Color.white, fontSize: FontSize.font1_size }}>Approved</Text>
          </TouchableOpacity>
        ...
        <ScrollView showsVerticalScrollIndicator={false} style={{ marginBottom: 120 }} >
          {report?.length > 0 ? report.filter((item) => {
            ...
            if (sel === "Pending") return item.leaveStatus === 'Pending';
            if (sel === "Rejected") return item.leaveStatus === 'Rejected';
            if (sel === "Pending") return item.leaveStatus === 'Pending';
            if (sel === "Approved") return item.leaveStatus === 'Approved';
            return true;
          }).map(i => {
            return (<>
              ...
              <View style={{ marginVertical: 5, marginHorizontal: 10 }}>
                <View style={{ padding: 15, borderRadius: 20, width: '100%', elevation: 5, backgroundColor: Color.white }}>
                  ...
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', width: '100%' }}>
                    <View style={{ width: '70%' }}>
                      ...
                      <Text style={{ color: Color.gray2, fontSize: FontSize.font_size, fontWeight: 'bold' }}>
                        ...
                        <Text style={{ color: Color.gray2, fontSize: FontSize.font_size, fontWeight: '400' }}>
                          {moment(i?.startLeaveDateTime).format('YYYY-MM-DD')}
                        </Text>
                      </Text>
                      ...
                      <Text style={{ color: Color.gray2, fontSize: FontSize.font_size, fontWeight: 'bold' }}>
                        🏝️ Leave End Date:
                        <Text style={{ color: Color.gray2, fontSize: FontSize.font_size, fontWeight: '400' }}>
                          {moment(i?.endLeaveDateTime).format('YYYY-MM-DD')}
                        </Text>
                      </Text>
                      ...
                      {i?.leaveReason ?
                        <Text style={{ color: Color.gray2, fontSize: FontSize.font_size, fontWeight: 'bold' }}>
                          📝 Leave Reason:
                          <Text style={{ color: Color.gray2, fontSize: FontSize.font_size, fontWeight: '400' }}>
                            {i?.leaveReason ? i?.leaveReason : 'Not Available'}
                          </Text>
                        </Text>
                        : null}
                    ...
                    <TouchableOpacity style={[tw`p-2`, { backgroundColor: i?.leaveStatus === 'Pending' ? Color.gray2 : i?.leaveStatus === 'Rejected' ? Color.danger : Color.colorMediumseagreen, borderRadius: 20, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', width: 70 }]}>
